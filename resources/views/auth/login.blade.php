...
<div class="max-w-lg mx-auto p-6 bg-white rounded-md shadow-md mt-6">
    @if ($errors->any())
       <p class="bg-red-100 text-red-500 p-3 font-bold">{{$errors->first()}}</p> 
    @endif
    <form action="{{Route('registration.login')}}" class="mt-4" method="POST">
        @csrf
<div class="mb-4">
...
<p class="my-2">Pas un compte? <a href="{{Route('registration.register')}}" class="text-red-500">sign up </a></p>
</form>
</div>
 @endsection
